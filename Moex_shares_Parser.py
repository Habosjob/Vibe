...
def setup_logging():
    ...
    log_filename = os.path.join(log_dir, "moex_shares_download.log")
...
def analyze_and_find_header(raw_text, logger):
    ...
    Возвращает номер строки с заголовком и параметры для парсинга.
    """
...
def download_moex_shares_data(url, logger):
    """Загружает и корректно обрабатывает данные по акциям с MOEX."""
    try:
        logger.info("Начало загрузки данных по акциям...")
...
def save_to_excel_fixed(df, filename='moex_shares_rates.xlsx', logger=None):
    ...
    try:
        ...
        with pd.ExcelWriter(filename, engine='xlsxwriter') as writer:
            ...
            # Сохраняем данные
            df.to_excel(writer, sheet_name='SharesRates', index=False)
            
            # Настройка ширины столбцов (опционально)
            worksheet = writer.sheets['SharesRates']
...
# ==================== ОСНОВНАЯ ФУНКЦИЯ ====================
def main():
    """Главная функция скрипта для загрузки данных по акциям."""
    script_start = time.time()
    logger = setup_logging()
    
    logger.info("=" * 60)
    logger.info("ЗАГРУЗЧИК ДАННЫХ ПО АКЦИЯМ MOEX")
    logger.info("=" * 60)
    
    # URL для загрузки данных по акциям
    shares_url = "https://iss.moex.com/iss/apps/infogrid/stock/rates.csv?sec_type=stock_preferred_share,stock_common_share&iss.dp=comma&iss.df=%25d.%25m.%25Y&iss.tf=%25H:%25M:%25S&iss.dtf=%25d.%25m.%25Y%20%25H:%25M:%25S&iss.only=rates&limit=unlimited&lang=ru"
    
    logger.info(f"Загружаем данные по URL: {shares_url}")
    
    # 1. Загрузка данных
    logger.info("Этап 1: Загрузка данных с MOEX...")
    df = download_moex_shares_data(shares_url, logger)
    
    if df is None or df.empty:
        logger.error("Не удалось загрузить данные по акциям!")
        return
    ...
    if success:
        logger.info("=" * 60)
        logger.info("УСПЕХ! Данные по акциям сохранены без искажений кодировки")
        logger.info("=" * 60)
        
        # Вывод в консоль
        print(f"\n✅ ДАННЫЕ ПО АКЦИЯМ УСПЕШНО СОХРАНЕНЫ")
        print(f"   Файл: moex_shares_rates.xlsx")
        print(f"   Размер: {df.shape[0]} строк, {df.shape[1]} столбцов")
        print(f"   Пример столбцов: {list(df.columns)[:3]}...")
        
        # Анализ типов акций (если есть соответствующие столбцы)
        if 'TYPENAME' in df.columns:
            print(f"   Типы акций в данных: {df['TYPENAME'].unique()}")
        if 'SECID' in df.columns:
            print(f"   Пример тикеров: {list(df['SECID'].head(5))}")
    else:
