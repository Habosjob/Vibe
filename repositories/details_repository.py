...
class DetailsRepository:
    ...
    def load_cached_records_bulk(
        ...
        pair_placeholders = ",".join(["(?, ?)"] * (len(secids) * len(endpoints)))
        pair_values: list[str] = []
        for endpoint in endpoints:
            for secid in secids:
                pair_values.extend([endpoint, secid])
        ...
        fresh_query = f"""
            ...
            FROM details_cache
            WHERE fetched_at >= ?
            AND (endpoint, secid) IN ({pair_placeholders})
        """
        ...
        latest_query = f"""
            ...
            FROM (
                ...
                FROM details_cache
                WHERE (endpoint, secid) IN ({pair_placeholders})
            )
            WHERE rn = 1
        """
        ...
        with sqlite3.connect(self.db_path) as connection:
            for endpoint, secid, payload_raw, fetched_at in connection.execute(fresh_query, [cutoff, *pair_values]).fetchall():
                fresh_records[(endpoint, secid)] = (json.loads(payload_raw), fetched_at)

            for endpoint, secid, payload_raw, fetched_at in connection.execute(latest_query, pair_values).fetchall():
