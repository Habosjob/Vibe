...
def test_orderbook_fallback_from_marketdata(monkeypatch, tmp_path: Path) -> None:
    ...
    assert float(orderbook_frame.iloc[0]["top_of_book_best_bid"]) == 99.95
    assert float(orderbook_frame.iloc[0]["top_of_book_best_offer"]) == 100.05
    assert float(orderbook_frame.iloc[0]["top_of_book_spread"]) == pytest.approx(0.10)
