...
def test_parse_bond_payload_uses_text_fallback_when_labels_missing() -> None:
    html = """
    <div>Карточка</div>
    <p>Цена last bid ask 97.10 / 96.40 / 97.55</p>
    <p>Флоатер, привязка к индексу RUONIA + 1,15</p>
    <p>Событие в ближ дату: оферта put</p>
    <p>Дата расчета YTM 07.10.2029</p>
    """

    payload = DohodEnricher.parse_bond_payload(html)

    assert payload.ask_price == 97.55
    assert payload.index_name == "RUONIA"
    assert payload.index_spread == 1.15
    assert payload.event_name.startswith("событие")
    assert payload.ytm_date == "2029-10-07"
