...
def test_batch_save_replaces_old_rows(tmp_path: Path) -> None:
    ...
    save_cashflows_batch_to_db(
        ...
        cashflows_by_isin={
            "RU000A000001": [
                CashflowRecord(isin="RU000A000001", date=date(2026, 1, 1), kind="coupon", amount=10.0, rate=5.0)
            ]
        },
        source="moex",
    )

    saved = save_cashflows_batch_to_db(
        session_factory,
        cashflows_by_isin={
            "RU000A000001": [
                CashflowRecord(isin="RU000A000001", date=date(2027, 1, 1), kind="redemption", amount=1000.0, rate=100.0)
            ],
            "RU000A000002": [
                CashflowRecord(isin="RU000A000002", date=date(2026, 6, 1), kind="coupon", amount=40.0, rate=8.5)
            ],
        },
        source="moex",
    )
    ...
    save_derived_fields_batch_to_db(
        session_factory,
        derived_by_isin={
            "RU000A000001": derive_fields(
                [CashflowRecord(isin="RU000A000001", date=date(2027, 1, 1), kind="redemption", amount=1000.0, rate=100.0)],
                today=date(2026, 1, 1),
            ),
            "RU000A000002": derive_fields(
                [CashflowRecord(isin="RU000A000002", date=date(2026, 6, 1), kind="coupon", amount=40.0, rate=8.5)],
                today=date(2026, 1, 1),
            ),
        },
    ...
    with session_factory() as session:
        rows = session.execute(select(Cashflow).order_by(Cashflow.isin, Cashflow.date)).scalars().all()
        fields = session.execute(select(InstrumentField).order_by(InstrumentField.isin, InstrumentField.field)).scalars().all()

    assert [(row.isin, row.kind) for row in rows] == [
        ("RU000A000001", "redemption"),
        ("RU000A000002", "coupon"),
    ]
    assert len(fields) == 8
