...
- Назначение: постраничный запрос облигаций с ретраями и задержками; парсит все поля `securities`, которые вернула MOEX, и обогащает бумаги полем `Amortization_start_date` по endpoint `bondization`.
