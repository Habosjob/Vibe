...
- Назначение: сбор справочника эмитентов по итоговому списку облигаций: сначала сопоставляет `SECID -> EMITTER_ID` (только при необходимости), затем загружает карточки `emitters/{EMITTER_ID}` для полного наименования и ИНН, с выводом подэтапов прогресса в консоль и кэшированием рынков `bonds/shares` на 24 часа, чтобы ускорять повторный запуск.
