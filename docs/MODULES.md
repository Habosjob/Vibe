...
- Назначение: инкрементальное состояние (исключения + кэш допущенных бумаг + кэш статичных данных эмитентов) и чекпоинты выполнения. Поддерживает `storage_backend=json|sqlite`.
...
- Назначение: сбор справочника эмитентов по итоговому списку облигаций: полное наименование, ИНН, тикеры акций и ISIN облигаций, с выводом подэтапов прогресса в консоль и кэшированием рынков `bonds/shares` на 24 часа, чтобы ускорять повторный запуск.
