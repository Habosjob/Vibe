...
- Назначение: сбор справочника эмитентов по итоговому списку облигаций: сначала сопоставляет `SECID -> EMITTER_ID` (только при необходимости), затем загружает карточки `emitters/{EMITTER_ID}` для полного наименования и ИНН. Для полей тикеров/ISIN поддерживается fallback по `ISSUER_ID` и нормализация `EMITTER_ID` (в т.ч. значения вида `123.0`). Рынок `bonds` теперь загружается только когда реально нужен (есть пропуски `EMITTER_ID` или `ISIN`), что заметно ускоряет запуск.
