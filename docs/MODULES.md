...
Назначение: параллельное обогащение облигаций данными с analytics.dohod.ru с чекпоинтами и кешем на сутки (для `Scorerate=Redlist` — до 30 дней).
...
- Особенности: для бумаг с `COUPONPERCENT < 1` используется формула бескупонной облигации; `ACCRUEDINT` всегда учитывается в dirty price. Для флоатеров применяется прогноз купона от траектории ключевой ставки, для ОФЗ-ИН (52002/52003/52004/52005) — прогноз индексации номинала на инфляцию.
