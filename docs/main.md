...
- Excel-витрина ручного blacklist: `DroppedBonds.xlsx` (имя управляется `DROPPED_BONDS_XLSX_FILENAME`). Файл создается автоматически при сборке Screener, если отсутствует.
  - Лист: `DroppedBonds`; колонки: `ISIN`, `Комментарий`.
  - При загрузке blacklist ISIN нормализуется (trim + upper + удаление пробелов/служебных символов, остаются только `A-Z0-9`), пустые/невалидные значения игнорируются, дубликаты схлопываются в `set`.
  - Все ISIN из blacklist исключаются на SQL-этапе сборки Screener через `TEMP TABLE tmp_dropped_bonds`, поэтому они не попадают ни в SQL-таблицу `Screener`, ни в `Screener.xlsx`.
...
- Для формул флоатеров добавлена нормализация спецсимволов (`∑`, `Σ`, `?`) и смешанных алфавитов (`KC`/`КС`).
- Для G-Curve используется интерполяция по ZCYC-точкам ЦБ при неточном совпадении срока (например, `0.49y`).
- Парсеры ЦБ читают целевые таблицы (Key rate / RUONIA / ZCYC) и не обновляют TTL кэша при пустом payload.
