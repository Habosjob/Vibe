# Скрипт `main.py` — загрузка MOEX bonds + Доходъ bonds + рейтинги НРА/АКРА/НКР + витрина эмитентов
...
14. Синхронизирует ручные оценки из Excel-витрины `Emitents.xlsx` обратно в SQL.
15. Автоматически заполняет `DateScoring`, когда есть `Scoring`, а дата еще пустая.
16. Формирует Excel-витрину `Emitents.xlsx` в корне проекта с оформлением и валидацией `Scoring`.
17. Создает снапшоты в `BaseSnapshots`:
