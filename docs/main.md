...
- `YTM_PRICE_FACEVALUE_MULTIPLIER` — множитель нормализации цены в формуле YTM (`normalized_price = price * FACEVALUE * multiplier`; дефолт `1.0`, для цен «в % от номинала» обычно задают `0.01`).
...
- Правила построения:
  ...
  - `YTM` рассчитывается для подэтапа «Считаем фиксы» по правилам:
    ...
    - цена нормализуется как `price * FACEVALUE * YTM_PRICE_FACEVALUE_MULTIPLIER`, затем добавляется `НКД` (получаем «грязную цену покупки»);
    ...
    - используется стандартная модель YTM для фиксированного купона с дискретным начислением по `Купон (раз/год)`.
