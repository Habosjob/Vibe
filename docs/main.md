...
- Правила построения:
  ...
  - `YTM` рассчитывается для подэтапа «Считаем фиксы» по правилам:
    ...
    - если `Аморт = 1/Да/галочка`, используется стандартный YTM с учетом графика амортизаций из таблицы `MoexAmortizations`;
    - если `Суборд` и `Аморт` не отмечены, используется стандартный YTM для фиксированного купона с дискретным начислением по `КупонПериод` и дате ближайшего купона.
