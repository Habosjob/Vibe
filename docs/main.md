...
- `YTM_PRICE_FACEVALUE_MULTIPLIER` — множитель нормализации цены в формуле YTM (`price_share = price * multiplier`, затем `dirty_price = price_share * FACEVALUE + НКД`; дефолт `1.0`). Если после умножения `price_share > 2`, включается авто-нормализация `/100` для цен в процентах (`94.39` → `0.9439`).
...
- Правила построения:
  ...
  - `YTM` рассчитывается для подэтапа «Считаем фиксы» по правилам:
    - расчет выполняется только если `Суборд`, `Аморт`, `Лесенка` имеют отрицательное значение (`False/Нет/0/❌/крестик`);
    ...
    - цена сначала нормализуется до доли от номинала (`price_share = price * YTM_PRICE_FACEVALUE_MULTIPLIER`, а если `price_share > 2` — дополнительно `/100`), затем считается `dirty_price = price_share * FACEVALUE + НКД`;
    - дата погашения для расчета: `Offerdate`, а если она пустая — `MATDATE`;
    - `Купон, %` интерпретируется как целое процентное значение (например `18` => `0.18` в формуле);
    - используется стандартная модель YTM для фиксированного купона с дискретным начислением; поле `КупонПериод` интерпретируется как дни купонного периода (`182`→~2 выплаты/год, `91`→~4, `30`→~12). Если значение ≤12, оно трактуется как «раз в год».
