...
8. Ведет таблицу `emitents` с уникальными эмитентами по `INN` в основной БД:
   ...
   - хранит пользовательские поля `Scoring`, `DateScoring` и столбец `NRA_Rate`.
9. Обогащает витрину эмитентов рейтингами НРА:
   - матчинг по `INN`,
   - формат значения: `Рейтинг(прогноз)`.
10. Синхронизирует ручные оценки из Excel-витрины `Emitents.xlsx` обратно в SQL.
11. Автоматически заполняет `DateScoring`, когда есть `Scoring`, а дата еще пустая.
12. Формирует Excel-витрину `Emitents.xlsx` в корне проекта с оформлением и валидацией `Scoring`.
13. Создает снапшоты в `BaseSnapshots`:
...
14. Пишет технический лог в `logs/main.log` (перезаписывается каждый запуск).
