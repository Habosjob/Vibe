...
- `dohod_index_values`: опциональный словарь с актуальными значениями индексов (`RUONIA`, `CBR_RATE`, `Z_CURVE_RUS`, при необходимости `Z_CURVE_RUS_7Y`). Если `CBR_RATE` пустой/нулевой, скринер пытается автоматически подтянуть актуальную ключевую ставку ЦБ.
  - Если в карточке ДОХОД есть явное значение базы индекса (например, «Значение RUONIA ... 15,40%»), для конкретной бумаги используется это значение (приоритет выше конфигурируемого), что снижает риск пропуска расчета купона при неполном конфиге.
  - Допускаются значения как с точкой, так и с запятой (`15.5` и `15,5`).
  - Поддерживаются русские алиасы ключей: `Ключевая ставка ЦБ` -> `CBR_RATE`, `КБД ОФЗ` -> `Z_CURVE_RUS`.
- `cbr_key_rate_url`: URL источника ключевой ставки ЦБ для автоподстановки `CBR_RATE`.
- `cbr_key_rate_timeout_seconds`: таймаут запроса к источнику ключевой ставки.
...
- Если `RUONIA` и/или `Z_CURVE_RUS` не заданы (0/пусто), а `CBR_RATE` доступна (из конфига или авто-запроса), они временно подставляются из `CBR_RATE`, чтобы расчет `COUPONPERCENT` не останавливался на большинстве выпусков.
