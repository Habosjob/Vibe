...
- `providers.moex_iss.cashflows_concurrency` — параллельность загрузки расписаний выплат (по умолчанию `12`).
- `providers.moex_iss.rate_limit_per_sec` — ограничение частоты запросов к `iss.moex.com` (по умолчанию `15`).
