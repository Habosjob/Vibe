...
Используется URL `https://analytics.dohod.ru/bond/{ID}`, где `ID` берется в приоритете из `ISIN`, а при его отсутствии — из `SECID` (fallback для старых/неполных записей).
