...
class SQLiteCache:
    ...
    def _migrate_bonds_list_if_needed(self) -> None:
        ...
        if self.logger:
            self.logger.warning(f"Schema mismatch: bonds_list columns={sorted(have)} -> recreate/migrate")
        ...
        # найти колонку с json-пейлоадом в старой таблице
        payload_col = None
        ...
        if "asof_date" in old and payload_col:
            if created_col:
                # created_utc не NULL: если NULL в old — ставим now
                cur.execute(f"""
    ...
    def _migrate_bond_raw_if_needed(self) -> None:
        ...
        want = {
            "id", "secid", "kind", "asof_date", "url", "params_json", "status",
            "elapsed_ms", "size_bytes", "response_text", "created_utc",
        }
        ...
        if self.logger:
            self.logger.warning(f"Schema mismatch: bond_raw columns={sorted(have)} -> recreate/migrate")

        # rename current to old
        cur.execute("ALTER TABLE bond_raw RENAME TO bond_raw_old;")
        self.conn.commit()

        # create fresh bond_raw
        cur.execute("""
    ...
    def set_bond_raw(
        ...
        *,
        ...
        INSERT INTO bond_raw(
            secid, kind, asof_date,
            url, params_json, status, elapsed_ms, size_bytes, response_text, created_utc
        )
        VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            secid, kind, asof_date,
    ...
    def upsert_emitent(
        ...
        *,
        ...
        INSERT INTO emitents(
            emitter_id, inn, title, short_title, ogrn, okpo, raw_json, updated_utc
        )
        VALUES(?, ?, ?, ?, ?, ?, ?, ?)
        ...
        """, (
            int(emitter_id),
            inn, title, short_title, ogrn, okpo,
            raw_json,
            _utc_iso(),
        ))
        self.conn.commit()
