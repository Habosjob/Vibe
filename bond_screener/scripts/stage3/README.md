...
Если в DOM не хватает полей, включается fallback по JSON/script-данным страницы (по набору алиасов ключей вроде `currency`, `status`, `issue_date`, `ytm`, `coupon_*`). Это закрывает кейсы, когда карточка рисуется фронтендом и часть данных лежит в script-блоках.

Для HTML-таблиц используется первый **непустой** value-cell после ячейки с label (а не только второй столбец). Это нужно для страниц, где между label и фактическим значением есть служебная пустая колонка.
...
- TTL-проверка выполняется батчем одним SQL-запросом для всех кандидатов, без `SELECT` на каждый ISIN, чтобы ускорить подготовительный этап.
...
Дополнительно есть отдельная полоса `Stage3/DOHOD prepare` для этапа подготовки (валидация + TTL-checkpoint), чтобы пользователь видел активность до старта сетевых запросов.

- `logs/stage3_run.log`
- `logs/stage3_moex_export.log`
- `logs/stage3_dohod_export.log`

Если парсер нашёл слишком мало полей, в `stage3_dohod_export.log` пишется диагностическая запись `DOHOD parse degraded ...` с количеством пропусков, списком извлечённых ключей и HTML-snippet для дебага.
...
- `parse_debug_missing_threshold` — порог количества `missing:*`, после которого включается расширенный debug-лог
- `parse_debug_html_snippet_chars` — длина HTML-snippet в диагностическом логе
