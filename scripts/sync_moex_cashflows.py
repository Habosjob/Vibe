...
async def _run_sync(base_dir: Path, logger: logging.Logger) -> tuple[int, int, int]:
    ...
    concurrency = max(1, int(provider_cfg.get("cashflows_concurrency", 12)))
    rate_limit = float(provider_cfg.get("rate_limit_per_sec", 15.0))
    ...
    logger.info("Этап 3/4: сохранение cashflows и derived полей в SQLite (batch)")
    derived_by_isin = {isin: derive_fields(rows) for isin, rows in collected.items()}
    ...
    with session_factory() as session:
        ...
        derived_rows = session.execute(
            ...
            .where(
                InstrumentField.isin.in_(sample_isins),
                InstrumentField.field.in_(["maturity_date", "next_coupon_date", "amort_start_date", "has_amortization"]),
            )
