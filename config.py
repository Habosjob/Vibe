...
# Базовый спред между ключевой ставкой и RUONIA в п.п.,
# который используется как безопасный fallback,
# если онлайн-страница ЦБ недоступна или распарсилась некорректно.
YTM_RUONIA_KEY_SPREAD_DEFAULT = 0.2

# Порог здравого смысла для спредов индикаторов в п.п.
# Если абсолютный спред выше порога, расчёт YTM применяет безопасный fallback,
# чтобы не получать аномально высокие/низкие купоны и YTM.
YTM_MAX_ABS_SPREAD_SANITY = 5.0
