...
# Прогноз ключевой ставки ЦБ РФ для расчета YTM флоатеров.
# Ключ словаря — номер года от текущей даты расчета:
#   0 — текущий год, 1 — следующий, 2 — второй год вперед.
# Для горизонтов > 2 лет используется значение bucket=2.
# Значение: dict[int, float] (проценты годовых).
# По умолчанию: {0: 14.0, 1: 8.5, 2: 8.0}.
KEY_RATE_FORECAST = {0: 14.0, 1: 8.5, 2: 8.0}

# Прогноз инфляции для линкеров (ОФЗ-ИН / SU52*).
# Механика bucket аналогична KEY_RATE_FORECAST.
# Значение: dict[int, float] (проценты годовых).
# По умолчанию: {0: 5.4, 1: 4.0, 2: 4.0}.
INFLATION_FORECAST = {0: 5.4, 1: 4.0, 2: 4.0}

# Базовые URL страниц ЦБ РФ для индикаторов флоатеров.
# По умолчанию используются англоязычные страницы hd_base.
CBR_KEY_RATE_URL = "https://www.cbr.ru/eng/hd_base/KeyRate/"
CBR_RUONIA_URL = "https://www.cbr.ru/eng/hd_base/ruonia/"
CBR_ZCYC_URL = "https://www.cbr.ru/eng/hd_base/zcyc_params/"

# TTL кэша индикаторов ЦБ в SQLite.
# Значение: целое число > 0 (часы).
# По умолчанию: 12.
